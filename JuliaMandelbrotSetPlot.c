       ...
       for(long double x=-1.5;x<=1.5;x+=0.01)
       {
           for(long double y=-1.5;y<=1.5;y+=0.01)
           {
                   ...
                   for(int i=0;i<iteration;i++)
                   {
                           ...
                           {
                             k=i+2-log2l(log2l(r));
                             sprintf(str,"{RGBColor[%Lf,%Lf,%Lf],Point[{%Lf,%Lf}]},",0.5*(1+sinl(k)),0.5*(1+sinl(k+2.*pi/3.)),0.5*(1+sinl(k+4*pi/3.)) ,x,y);
                             fputs(str,file);
                               break;
                           }
                   }
                    sprintf(str,"{Point[{%Lf,%Lf}]},",x,y);
                    fputs(str,file);
           }
       }
       fputs("Nothing}]",file);
...
void MandelbrotSetPlot(struct Complex *z,char *FileName,int iteration)
{
     ...
     struct Complex zn;
       long double re,im,r,k;
        char str[200];
       fputs("Graphics[{",file);
       for(long double x=-2;x<=1.0;x+=0.01)
       {
           for(long double y=-1.5;y<=1.5;y+=0.01)
           {
